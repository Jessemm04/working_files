...
app.config['SECRET_KEY'] = 'mysecretkey' #used so session cookies can be encryptec
...
def check_for_winner(player):
    ...
    #check rows
    for i in range(3):
        if session['board'][i][0] == player and session['board'][i][1] == player and session['board'][i][2] == player:
            result = True
    #check columns
    if result == False:
        for i in range(3):
            if session['board'][0][i] == player and session['board'][1][i] == player and session['board'][2][i] == player:
                result = True

    #check diagonals
    if result == False:
        if session['board'][0][0] == player and session['board'][1][1] == player and session['board'][2][2] == player:
            result = True
        else:
            if session['board'][2][0] == player and session['board'][1][1] == player and session['board'][0][2] == player:
                result = True
    # return result
    return result
